  ...
  for frameIdx=params.start_number + params.continue_with - 1, params.start_number + num_images - 1 do
    ...
    elseif init == 'prevWarped' and frameIdx > params.start_number then
	  ...
	  if flag==1 then
		  ...
		  img = image.load(string.format(params.content_pattern, frameIdx), 3)
		  last = image.load(string.format(params.content_pattern, frameIdx-1), 3)
		  --[[
		  --flow resize--
		  local img_size=math.max(img:size(2),img:size(3))
		  local flow_new=torch.Tensor(3,flow:size(2),flow:size(3)):zero()
		  flow_new[1]=flow[1]
		  flow_new[2]=flow[2]
		  flow_new=image.scale(flow_new,img_size,'bilinear')
		  print(flow:size())
		  print(flow_new:size())
		  ]]
		  ...
		  img=myinit(last,flow,img,last_style,weightsFileName,sizescale,sim_thresh,map_size,sample_ratio,map_cycle)
		  --img=myinit(last,flow_new[{{1,2},{},{}}],img,last_style,weightsFileName,sizescale,sim_thresh,map_size,sample_ratio,map_cycle)
	  else
	  --change end--
    ...
    if frameIdx > params.start_number and params.temporal_weight ~= 0 then
      ...
      for j=1, #J do
        local prevIndex = J[j]+1--now we do not read last picture and warp it,just get this frame,so +1
		...
		local fileName = string.format('%sinit-' .. params.number_format .. '.png',
          params.output_folder, math.abs(prevIndex - params.start_number + 1))
		 
		imgWarped = image.load(fileName, 3)
		imgWarped = preprocess(imgWarped):float()
        imgWarped = MaybePutOnGPU(imgWarped, params)
		table.insert(imgsWarped, imgWarped)
      end
	...
	end_time0 = os.time()
	elapsed_time = os.difftime(end_time0-start_time0)
	print("Total Iteration Running time: " .. elapsed_time .. "s")
  end
end
